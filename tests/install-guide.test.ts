...
describe("agent installation instructions", () => {
 ...
 it("keeps the README pointed at the managed installation and configuration profile", () => {
  ...
  expect(readme).toContain("[Agent installation](agents-install.md)");
 });
 ...
 it("defines a reproducible and all-enabled installation", () => {
  ...
  const requiredText = [
   "Use Bun",
   `@oh-my-pi/pi-coding-agent@${JSON.parse(rootFile("package.json")).devDependencies["@oh-my-pi/pi-coding-agent"]}`,
   ...
   "scripts/check-sol-omp-config.mjs",
   "omp plugin list",
   "tests/all-mechanisms.test.ts",
  ];

  for (const text of requiredText) expect(guide).toContain(text);
  expect(guide).toMatch(/do not modify, patch, fork, or vendor OMP/is);
