...
import { findCutPoint } from "@oh-my-pi/pi-agent-core/compaction";
import {
	buildSessionContext,
...
function entryHasContext(entry: SessionEntry): boolean {
...
function compactionMessageCount(entries: readonly SessionEntry[], startIndex: number, endIndex: number): number {
	...
	for (let index = startIndex; index < endIndex; index++) {
		...
		if (entry && entry.type !== "compaction" && entryHasContext(entry)) count++;
	}
	return count;
}
...
function branchAfterAbort(entries: readonly SessionEntry[]): SessionEntry[] {
	...
	return [
		...
		{
			type: "message",
			id: "sol-omp-online-context-compact-abort-marker",
...
function nativeCompactionFeasible(entries: readonly SessionEntry[], keepRecentTokens: number): boolean {
	...
	const cut = findCutPoint(path, tokenizer, startIndex, path.length, keepRecentTokens);
...
export function createOnlineContextCompactExtension(options: OnlineContextCompactOptions = {}): ExtensionFactory {
	...
	return (pi) => {
		...
		const contextTokens = (context: ExtensionContext): number => {
			const visible = observedMessages.reduce((total, message) => total + estimateMessageTokens(message), 0);
			const systemPrompt = context.getSystemPrompt().join("\n");
			const estimated = visible + tokenEstimate(systemPrompt);
		...
		pi.on("session_stop", async (event, context) => {
			...
			if (!compacted || event.signal.aborted) return;
			pendingContinuation = true;
			return { continue: true, additionalContext: POST_COMPACTION_PLAN_REMINDER };
		});
